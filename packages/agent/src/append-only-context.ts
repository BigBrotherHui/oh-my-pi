...
export class StablePrefix {
	...
	// Prompt bytes snapshot: the prompt array is caller-owned and mutable
	// in place (Agent.setSystemPrompt stores the caller's array; anyone
	// holding it can push/splice), so reference equality alone cannot prove
	// the bytes are unchanged. Stored as a per-segment copy and compared
	// element-wise — a joined string cannot encode boundaries ("a\0b" vs
	// ["a", "b"] collide under any single-char delimiter).
	#lastPromptText: readonly string[] | undefined;
	...
	#fastPathHit(context: AgentContext, options: BuildOptions): boolean {
		...
		// Prompt by reference first (steady state), then element-wise so
		...
		if (this.#lastPromptText !== undefined) {
			if (this.#lastPromptText.length !== context.systemPrompt.length) return false;
			for (let i = 0; i < this.#lastPromptText.length; i++) {
				if (this.#lastPromptText[i] !== context.systemPrompt[i]) return false;
			}
		}
	...
	#recordFastPathKey(context: AgentContext, options: BuildOptions, normalized: Tool[]): void {
		...
		this.#lastPromptText = [...context.systemPrompt];
		this.#lastIntentTracing = options.intentTracing;
		this.#lastPruneToolDescriptions = options.pruneToolDescriptions;
