...
describe("StablePrefix", () => {
	...
	it("skips normalize+stringify when live references are unchanged", () => {
		...
		// No rebuild: the fast path reports unchanged, and the served
		// context still carries the same bytes (deep equality, not object
		// identity — a refactor may rebuild equivalent frozen arrays).
		expect(p.build(ctx, BUILD_OPTS)).toBe(false);
		expect(p.toContext().tools).toEqual(first.tools);
		expect(p.toContext().systemPrompt).toEqual(first.systemPrompt);
	});
	...
	it("distinguishes delimiter collisions across prompt boundaries", () => {
		// ["a\0b"] vs ["a", "b"]: identical under join("\0") and equal
		// length tools/options, so the old joined key returned false and
		// kept serving the stale frozen prompt. Element-wise compare misses.
		const p = new StablePrefix();
		const ctx1 = makeContext({ systemPrompt: ["a\0b"] });
		expect(p.build(ctx1, BUILD_OPTS)).toBe(true);
		expect(p.build(ctx1, BUILD_OPTS)).toBe(false);
		const ctx2 = makeContext({ systemPrompt: ["a", "b"] });
		expect(p.build(ctx2, BUILD_OPTS)).toBe(true);
		expect(p.toContext().systemPrompt).toEqual(["a", "b"]);
	});
