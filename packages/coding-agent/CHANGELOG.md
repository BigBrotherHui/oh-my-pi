...
- Fixed the Wayland `computer` input backend (libei) emitting fixed US keycodes, so on non-US layouts (e.g. French AZERTY) `press`/`type` inserted the wrong characters and accented characters like `é` failed with `InputFailed`; keystrokes now resolve through the compositor's active XKB keymap, including required modifiers such as AltGr ([#12534](https://github.com/can1357/oh-my-pi/issues/12534)).
