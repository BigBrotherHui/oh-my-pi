...
import { ALL_SEGMENT_IDS, statusSegmentNode } from "@oh-my-pi/pi-tui/status-line/segments";
...
function renderIsolatedSegment(id: StatusLineSegmentId, spec: SegmentVariantSpec, width: number): readonly string[] {
	...
	const segment = statusSegmentNode(id, context);
	const content = (
		<text width={Math.max(1, width - 2)} wrap="none" overflow="ellipsis">
			{segment ?? <span color="dim">(hidden for this sample)</span>}
		</text>
	);
	const rows = renderSnapshot(() => content, { columns: Math.max(1, width - 2) });
	const indent = "  ";
	if (rows.length === 0) return [indent];
