...
describe("issue #4806 command output during streaming", () => {
	...
	it("mounts slash-command output once after the active turn ends", async () => {
		mode.chatContainer.append({ id: "streaming-reply", view: () => <text>agent is streaming</text> });

		mode.handleToolsCommand();

		expect(mode.chatContainer.entries()).toHaveLength(1);

		streaming = false;
		await mode.eventController.handleEvent({ type: "agent_end", messages: [] } as AgentSessionEvent);

		expect(mode.chatContainer.entries()).toHaveLength(2);
		const transcript = renderToRows(() => <TranscriptView store={mode.chatContainer} />, 80).join("\n");
	...
	it("drops deferred slash-command output when the session changes before agent_end", async () => {
		mode.chatContainer.append({ id: "old-streaming-reply", view: () => <text>old session is streaming</text> });
		...
		expect(mode.chatContainer.entries()).toHaveLength(1);
		expect(mode.chatContainer.entries()[0]?.id).toBe("old-streaming-reply");
	});
});
