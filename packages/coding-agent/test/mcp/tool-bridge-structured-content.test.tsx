...
describe("MCP result rendering with structuredContent", () => {
	async function renderText(result: MCPToolCallResult): Promise<string> {
		const built = await build(result);
		const call = createToolCallModel<{}, MCPToolDetails>({ id: "call-1", toolName: "mcp", label: "list_issues" });
		call.applyResult(built);
		call.setUi({ expanded: true });
		return Bun.stripANSI(renderToRows(() => <McpToolView {...call} />, 160).join("\n"));
	}
