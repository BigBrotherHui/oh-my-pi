...
function terminalStateRows(info: TerminalStateInfo): readonly string[] {
	return [
		"Terminal State",
		"━━━━━━━━━━━━━━",
...
export function TerminalStateView({ info }: { info: TerminalStateInfo }): JSX.Element {
	return <text wrap="word">{formatTerminalState(info)}</text>;
}

/** Compatibility wrapper for non-run consumers. */
export function formatTerminalState(info: TerminalStateInfo): string {
	return terminalStateRows(info).join("\n");
}
