 ...
 */
/** Slim divider after a warm prompt cache becomes cold. */
export function CacheInvalidationMarkerView({ info }: { readonly info: CacheInvalidation }): JSX.Element {
	const { symbol } = useTheme();
	const label = createMemo(() => {
		const icon = symbol("icon.cacheMiss");
		const head = icon ? `${icon} cache miss` : "cache miss";
		return info.reprocessedTokens > 0
			? `${head} ${symbol("sep.dot").trim()} ${formatNumber(info.reprocessedTokens)} tokens`
			: head;
	});
