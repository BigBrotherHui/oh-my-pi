...
import { createMemo, useTheme, type JSX } from "../reactive";
 ...
 */
/** Divider that discloses a provider-side model substitution. */
export function ServedModelMarkerView({ info }: { readonly info: ServedModelMismatch }): JSX.Element {
	const { symbol } = useTheme();
	const via = info.upstreamProvider ? `${info.provider}/${info.upstreamProvider}` : info.provider;
	const label = createMemo(() => {
		const dot = symbol("sep.dot").trim();
		return `${symbol("status.warning")} served ${info.served} ${dot} requested ${info.requested} ${dot} via ${via}`;
	});
	return (
		<stack>
			<br />
