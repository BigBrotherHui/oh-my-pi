...
import { Clip, over, spaces, takeCells } from "../core/out";
...
export interface EditorTheme {
	...
	/** Style for inline hint and ghost text after the cursor. */
	hintStyle?: Style;
}
...
/** Source location for one visual text segment passed to `decorateRuns`. */
...
/** Native cursor decoration; animated colors are driven by the mounted host clock. */
export interface EditorCursor {
	readonly text: string;
	readonly style?: Style;
	readonly rainbow?: boolean;
	/** Suppress the cursor glyph for a read-only presentation of this editor. */
...
export interface EditorViewProps {
	key?: NodeKey;
	editor: Editor;
	/** Retained status content embedded in the editor's native top chrome. */
	topBorder?: JSX.Element;
	cursor?: EditorCursor;
	/** Read-only sample text fitted to the editor's native one-row content width. */
	previewText?: string;
	/** Negative values keep static preview editors out of the focus order. */
	tabIndex?: number;
}

/** Run-based editor view for TSX consumers. */
...
export class Editor {
	/** Optional semantic style for the configured prompt gutter. */
	promptGutterStyle?: Style;
	#state: EditorState = {
		lines: [""],
		cursorLine: 0,
		cursorCol: 0,
	...
	/** Style of the optional cursor override glyph. */
	cursorOverrideStyle: Style | undefined;
	/** Display width of the cursorOverride glyph. */
	cursorOverrideWidth: number | undefined;
	/** Run-native hook that decorates displayed user text after source-text layout. */
	decorateRuns: ((out: Out, text: string, context: EditorTextDecorationContext, base: Style) => void) | undefined;
	...
	#borderPaintStyle = Style.NONE;
	/** Current border color; updates notify every mounted editor surface. */
	get borderStyle(): Style {
		return this.#borderPaintStyle;
	}
	set borderStyle(style: Style) {
		if (this.#borderPaintStyle === style) return;
		this.#borderPaintStyle = style;
		this.invalidate();
	}
	...
	#autocompleteMenu?: AutocompleteMenuState;
	...
	// Placeholder chrome cached before the live retained status slot mounts.
	#topBorderContent?: EditorTopBorder;
	#borderVisible = true;
	#borderStyle: EditorBorderStyle = "box";
	constructor(theme: EditorTheme) {
		this.#theme = theme;
		this.borderStyle = theme.borderStyle;
	}
	...
	/** Current popup state for the retained editor host. */
	autocompleteMenu(): AutocompleteMenuState | undefined {
		return this.#autocompleteMenu;
	}
	...
	/** Install cached startup chrome until a live retained top-border slot takes precedence. */
	...
	setPromptGutter(promptGutter: string | undefined): void {
		if (this.#promptGutter === promptGutter) return;
	...
	/** Fit a static sample draft using native composer chrome before the host paints it. */
	setPreviewText(text: string, columns: number): void {
		const contentWidth = this.#getContentWidth(columns, this.#getEditorPaddingX());
		const fitted = truncateToWidth(text, Math.max(1, contentWidth - 1));
		const preview = this.#borderStyle === "box" ? `${fitted} ` : fitted;
		if (!this.textEquals(preview)) this.setText(preview);
	}
	...
	setPaddingX(paddingX: number): void {
		const value = Math.max(0, paddingX);
		if (this.#paddingXOverride === value) return;
		this.#paddingXOverride = value;
		this.invalidate();
	}
	...
	setAutocompleteMaxVisible(maxVisible: number): void {
		...
		if (this.#autocompleteMaxVisible !== newMaxVisible) {
			...
			this.#setAutocompleteMaxVisible(newMaxVisible);
			this.invalidate();
		}
	}
	...
	/** Subscribe a mounted host element to asynchronous editor and draft changes. */
	...
	#publishAutocomplete(): void {
		this.invalidate();
		const notify = this.onAutocompleteUpdate;
		notify?.();
	}

	/** Notify mounted editor surfaces that text, cursor, or completions changed. */
	invalidate(): void {
	...
	#getPromptGutter(
		...
		return {
			firstLine: takeCells(gutter, gutterWidth),
			continuation: spaces(gutterWidth),
			width: gutterWidth,
		};
	}
	...
	/** Paint user text through the optional run-native decorator. */
	#paintDecorated(out: Out, text: string, context: EditorTextDecorationContext, base: Style): void {
		if (text.length === 0) return;
		if (this.decorateRuns) this.decorateRuns(base === Style.NONE ? out : over(out, base), text, context, base);
		else out.push(base, text);
	}

	/** Style of the software cursor cell at the current modal state. */
	#cursorCellStyle(base: Style): Style {
		const insertShape = this.#vim !== null && this.#vim.mode === "insert";
		return base.plus(insertShape ? Attr.Underline : Attr.Inverse);
	}

	#getInputCursor(base: Style): { text: string; width: number; style: Style } {
		if (this.#vim !== null && this.#vim.mode !== "insert") {
			return { text: " ", width: 1, style: base.plus(Attr.Inverse) };
		}
		const text = this.#theme.symbols.inputCursor;
		return { text, width: visibleWidth(text), style: base };
	}

	#fitRuns(source: RichText, width: number, ellipsis: Ellipsis): RichText {
		const fitted = new RichText();
		source.replay(new Clip(fitted, Math.max(0, width), ellipsis));
		fitted.finish();
		return fitted;
	}

	#cursorCandidate(text: string, style: Style, maxWidth: number): RichText {
		const source = new RichText();
		source.push(style, text);
		source.br();
		return this.#fitRuns(source, maxWidth, Ellipsis.Omit);
	}

	#paintEndOfLineCursorAtWidthLimit(
		out: Out,
		before: string,
		maxWidth: number,
		context: EditorTextDecorationContext,
		base: Style,
		replacement?: { text: string; width: number; style?: Style },
	): number {
		const beforeGraphemes = [...segmenter.segment(before)];
		const lastGrapheme = beforeGraphemes[beforeGraphemes.length - 1]?.segment;
		const lastGraphemeWidth = lastGrapheme ? visibleWidth(lastGrapheme) : 0;
		const builtIn = this.#getInputCursor(base);
		const fallback = lastGrapheme
			? this.#cursorCandidate(lastGrapheme, this.#cursorCellStyle(base), maxWidth)
			: this.#cursorCandidate(builtIn.text, builtIn.style, maxWidth);
		let candidate = replacement
			? this.#cursorCandidate(replacement.text, replacement.style ?? base, maxWidth)
			: fallback;
		let candidateWidth = candidate.rowWidth[0] ?? 0;
		if (replacement && candidateWidth === 0) {
			candidate = fallback;
			candidateWidth = candidate.rowWidth[0] ?? 0;
		}
		if (lastGrapheme && candidateWidth === 0) {
			candidate = this.#cursorCandidate(builtIn.text, builtIn.style, maxWidth);
			candidateWidth = candidate.rowWidth[0] ?? 0;
		}

		const declaredWidth = replacement && candidateWidth > 0 ? replacement.width : candidateWidth;
		const replacedSpanWidth = Math.min(maxWidth, Math.max(lastGraphemeWidth, declaredWidth));
		const prefixWidth = Math.max(0, maxWidth - replacedSpanWidth);
		const prefix = takeCells(before, prefixWidth);
		this.#paintDecorated(out, prefix, { ...context, endCol: context.startCol + prefix.length }, base);
		const pad = Math.max(0, replacedSpanWidth - candidateWidth);
		if (pad > 0) out.push(base, spaces(pad));
		candidate.replayRow(out, 0);
		if (this.focused) out.cursor();
		return visibleWidth(prefix) + replacedSpanWidth;
	}

	#paintTerminalCursorLine(
		out: Out,
		text: string,
		maxWidth: number,
		context: EditorTextDecorationContext,
		...
		if (!this.focused) {
			this.#paintDecorated(out, text, context, base);
			return;
		}
		...
		let offset = 0;
		for (const seg of segmenter.segment(text)) {
			if (visibleWidth(seg.segment) > 0) insertAt = offset;
			offset += seg.segment.length;
		}
		this.#paintDecorated(out, text.slice(0, insertAt), { ...context, endCol: context.startCol + insertAt }, base);
		out.cursor();
		this.#paintDecorated(out, text.slice(insertAt), { ...context, startCol: context.startCol + insertAt }, base);
	}

	#paintHint(out: Out, hint: string, width: number, base: Style, style: Style | undefined): number {
		if (width <= 0 || hint.length === 0) return 0;
		const source = new RichText();
		source.push((style ?? Style.NONE.plus(Attr.Dim)).over(base), hint);
		source.br();
		const fitted = this.#fitRuns(source, width, Ellipsis.Unicode);
	...
	/** Paint editor chrome and content directly into the run pipeline. */
	paint(out: Out, width: number, header?: EditorTopBorder, cursor?: EditorCursor): void {
		width = Number.isFinite(width) ? Math.max(0, Math.trunc(width)) : 0;
		const cursorText = cursor?.text ?? this.cursorOverride;
		const cursorStyle = cursor?.style ?? this.cursorOverrideStyle;
		const cursorHidden = cursor?.hidden === true;
		const cursorWidth = cursor
			? visibleWidth(cursor.text)
			: (this.cursorOverrideWidth ?? (cursorText ? visibleWidth(cursorText) : 1));
		const style = this.#effectiveStyle();
		const paddingX = this.#getEditorPaddingX();
		...
		const topBorder = style.statusAttachment === "none" ? undefined : (header ?? this.#topBorderContent);

		const chromeCtx: ComposerChromeContext = {
			width,
			paddingX,
			borderStyle: this.borderStyle,
			accentStyle: this.#theme.accentStyle ?? this.borderStyle,
			surfaceStyle: this.#theme.surfaceStyle ?? Style.NONE,
			box: this.#theme.symbols.boxRound,
			topBorder,
		};
		let paintedRows = paintComposerTop(style, out, chromeCtx) ? 1 : 0;

		const lineContentWidth = contentAreaWidth;
		const inlineHint = this.#getInlineHint();
		const hintStyle = this.#theme.hintStyle;
		const vimSelection = this.#vimSelection();
		const baseTextStyle = isFilledComposerStyle(style) ? Style.NONE : (this.#theme.textStyle ?? Style.NONE);
		const rowScratch = new RichText();
		...
		for (let visibleIndex = 0; visibleIndex < visibleLayoutLines.length; visibleIndex++) {
			...
			const hasCursor = !cursorHidden && layoutLine.hasCursor && layoutLine.cursorPos !== undefined;

			if (!isSideBordered && displayWidth > lineContentWidth) {
				displayText = takeCells(displayText, lineContentWidth);
				displayWidth = visibleWidth(displayText);
			}

			if (!isSideBordered && lineContentWidth === 0) {
				const zero = new RichText();
				if (hasCursor && !this.#useTerminalCursor) {
					const budget = visibleWidth(gutterText);
					const replacement = cursorText
						? {
								text: cursorText,
								width: cursorWidth,
								style: cursorStyle ?? Style.NONE,
							}
						: this.#getInputCursor(Style.NONE);
					if (showPromptGutter && budget > 0) {
						...
						const remaining = Math.max(0, budget - promptGlyphWidth);
						if (remaining === 0) {
							zero.push(this.#cursorCellStyle(Style.NONE), promptGlyph);
							if (this.focused) zero.cursor();
						} else {
							zero.push(Style.NONE, promptGlyph);
							this.#paintEndOfLineCursorAtWidthLimit(
								zero,
								"",
								remaining,
								{ ...decorationContext, endCol: decorationContext.startCol },
								Style.NONE,
								replacement,
							);
						}
					...
					zero.push(Style.NONE, gutterText);
					if (hasCursor && this.focused) {
						if (this.#useTerminalCursor && gutterText.length > 0) {
							const last = [...segmenter.segment(gutterText)].findLast(seg => visibleWidth(seg.segment) > 0);
							if (last) {
								zero.clear();
								zero.push(Style.NONE, gutterText.slice(0, last.index));
								zero.cursor();
								zero.push(Style.NONE, gutterText.slice(last.index));
							} else {
								zero.cursor();
							}
						} else {
			...
			if (selectionSpan !== null) {
				...
				const cursorPos = layoutLine.cursorPos ?? 0;
				...
				if (this.#imeSafeCursorLayout && after.length === 0 && isSideBordered) {
					this.#paintDecorated(
						content,
						before,
						{ ...decorationContext, endCol: decorationContext.startCol + before.length },
						baseTextStyle,
					);
					if (this.focused) content.cursor();
					...
					if (this.focused) content.cursor();
					const available = Math.max(0, lineContentWidth - displayWidth);
					displayWidth += this.#paintHint(content, inlineHint, available, baseTextStyle, hintStyle);
				} else if (after.length === 0 && !isSideBordered && displayWidth >= lineContentWidth) {
					this.#paintTerminalCursorLine(content, before, lineContentWidth, decorationContext, baseTextStyle);
				} else {
					this.#paintDecorated(
						content,
						before,
					...
					this.#paintDecorated(
						content,
						after,
						{ ...decorationContext, startCol: decorationContext.startCol + before.length },
						baseTextStyle,
					);
				}
			} else if (hasCursor) {
				...
				const after = displayText.slice(cursorPos);
				if (after.length > 0) {
					const firstGrapheme = [...segmenter.segment(after)][0]?.segment ?? "";
					...
					if (this.focused) content.cursor();
					content.push(this.#cursorCellStyle(baseTextStyle), firstGrapheme);
					this.#paintDecorated(
						content,
						restAfter,
						{
					...
					const overrideWidth = cursorWidth;
					if (!isSideBordered && displayWidth + overrideWidth > lineContentWidth) {
						displayWidth = this.#paintEndOfLineCursorAtWidthLimit(
							content,
							before,
							lineContentWidth,
							decorationContext,
							baseTextStyle,
							{
								text: cursorText,
								width: overrideWidth,
								style: cursorStyle ?? baseTextStyle,
							},
						);
					} else {
						this.#paintDecorated(content, before, decorationContext, baseTextStyle);
						if (this.focused) content.cursor();
						content.push((cursorStyle ?? Style.NONE).over(baseTextStyle), cursorText);
					...
					const cursor = this.#getInputCursor(baseTextStyle);
					if (!isSideBordered && displayWidth + cursor.width > lineContentWidth) {
						displayWidth = this.#paintEndOfLineCursorAtWidthLimit(
							content,
							before,
							lineContentWidth,
						...
						this.#paintDecorated(content, before, decorationContext, baseTextStyle);
						if (this.focused) content.cursor();
						content.push(cursor.style, cursor.text);
						displayWidth += cursor.width;
						if (inlineHint) {
							const available = Math.max(0, lineContentWidth - displayWidth);
							displayWidth += this.#paintHint(content, inlineHint, available, baseTextStyle, hintStyle);
						}
				...
				this.#paintDecorated(content, displayText, decorationContext, baseTextStyle);
			}

			content.br();
			if (!hasCursor && (content.rowWidth[0] ?? 0) > lineContentWidth) {
				content = this.#fitRuns(content, lineContentWidth, Ellipsis.Unicode);
			}

			const prefix = style.sideChromeWidth(paddingX) > 0 ? `x${gutterText}` : gutterText;
			const measure = new RichText();
			if (prefix) measure.push(Style.NONE, prefix);
			const beforeContent = measure.openWidth;
			content.replayRow(measure, 0);
			displayWidth = measure.openWidth - beforeContent;
			const linePad = spaces(Math.max(0, lineContentWidth - displayWidth));

			rowScratch.clear();
			paintComposerRow(style, rowScratch, {
				...chromeCtx,
				text: content.rowText(0),
				content,
				contentRow: 0,
				pad: linePad,
				gutter: gutterText,
				gutterStyle: this.promptGutterStyle,
			...
			rowScratch.finish();
			paintedRows += rowScratch.rows;
			rowScratch.replay(out);
		}

		if (paintComposerBottom(style, out, chromeCtx)) paintedRows += 1;

		if (this.#autocompleteState !== null) {
			const viewportRows = this.viewportRowsProvider?.() || process.stdout.rows || Number(Bun.env.LINES) || 24;
			this.#setAutocompleteMaxVisible(
				Math.max(3, Math.min(this.#autocompleteMaxVisible, viewportRows - paintedRows - 2)),
			);
		}
	}
	...
	#handleInputChunk(data: string): string | undefined {
		...
		if (this.#autocompleteState && this.#autocompleteMenu) {
			...
			// Let the autocomplete list handle navigation and selection
				...
				if (
					...
					const page =
						...
						kb.matchesCanonical(canonical, "tui.select.pageDown");
					const direction =
						kb.matchesCanonical(canonical, "tui.select.up") || kb.matchesCanonical(canonical, "tui.select.pageUp")
							? -1
							: 1;
					this.#moveAutocompleteSelection(direction * (page ? this.#autocompleteMenu.maxVisible : 1));
				...
				if (kb.matchesCanonical(canonical, "tui.input.tab") || rightArrowAccepts) {
					const selected = this.#selectedAutocompleteItem();
	...
	/** Paint one Vim Visual selection row without serializing styles to ANSI. */
	#paintSelectedLine(
		out: Out,
		text: string,
		...
		const markerPos =
			!this.focused || cursorPos === undefined ? undefined : Math.max(0, Math.min(cursorPos, text.length));
		...
		for (let i = 0; i < points.length - 1; i++) {
			const from = points[i]!;
			const to = points[i + 1]!;
			if (from === markerPos) out.cursor();
			const segment = text.slice(from, to);
			if (from >= start && from < end) {
				out.push(base.plus(Attr.Inverse), segment);
			} else {
				this.#paintDecorated(
					out,
					segment,
		...
		if (markerPos !== undefined && markerPos >= text.length) out.cursor();
		if (span.trailingNewline) out.push(base.plus(Attr.Inverse), " ");
	}
	 ...
	 */
	#autocompletePrefixMatchesCursorText(currentTextBeforeCursor: string, item?: AutocompleteItem | null): boolean {
	...
	#selectedCompletionIsSkillNamespace(): boolean {
		return this.#selectedAutocompleteItem()?.value === SKILL_NAMESPACE;
	}
	...
	#createAutocompleteMenu(items: readonly AutocompleteItem[]): AutocompleteMenuState {
		return { items, selectedIndex: 0, offset: 0, maxVisible: this.#autocompleteMaxVisible };
	}

	#selectedAutocompleteItem(): AutocompleteItem | undefined {
	...
	#setAutocompleteMaxVisible(maxVisible: number): void {
		const menu = this.#autocompleteMenu;
		if (!menu || menu.maxVisible === maxVisible) return;
		this.#autocompleteMenu = {
			...menu,
			maxVisible,
			offset: Math.min(menu.offset, Math.max(0, menu.items.length - maxVisible)),
		};
	}

	#moveAutocompleteSelection(delta: number): void {
		const menu = this.#autocompleteMenu;
		if (!menu || menu.items.length === 0) return;
		const selectedIndex =
			(((menu.selectedIndex + delta) % menu.items.length) + menu.items.length) % menu.items.length;
		let offset = menu.offset;
		if (selectedIndex < offset) offset = selectedIndex;
		else if (selectedIndex >= offset + menu.maxVisible) offset = selectedIndex - menu.maxVisible + 1;
		this.#autocompleteMenu = { ...menu, selectedIndex, offset };
	}
	...
	async #showSpellingSuggestions(): Promise<void> {
		...
		this.#autocompleteMenu = this.#createAutocompleteMenu(replacements.items.map(value => ({ value, label: value })));
		this.#autocompleteState = "assist";
	...
	async #runAutocompleteRequest(request: AutocompleteRequest, requestId: number, signal: AbortSignal): Promise<void> {
		...
		if (suggestions && Array.isArray(suggestions.items) && suggestions.items.length > 0) {
			...
			this.#autocompleteMenu = this.#createAutocompleteMenu(suggestions.items);
