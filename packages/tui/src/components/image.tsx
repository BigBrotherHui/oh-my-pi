import { getImageDimensions, type ImageDimensions, type TERMINAL } from "../terminal-capabilities";
...
export interface ImageOptions {
	...
	/**
	 * Reserve a fixed terminal-cell rectangle for this image. The host centers
	 * the rendered protocol output or fallback within it and clips to a narrower
	 * parent without allowing the image to escape its layout allocation.
	 */
	cellBox?: {
...
export interface ImagePaintState {
	base64Data: string;
	mimeType: string;
	...
	cachedImageProtocol: typeof TERMINAL.imageProtocol;
	cachedCellWidthPx: number;
	cachedCellHeightPx: number;
	cachedKittyUnicodePlaceholders: boolean;
	renderedGraphicRows: number;
}

/** Props for the retained generic image host. */
export interface ImageProps {
	/** Prepared image state retained by the public view or compatibility facade. */
	readonly state: ImagePaintState;
}

export interface ImageViewSourceProps {
	base64Data: string;
	mimeType: string;
	theme: ImageTheme;
	options?: ImageOptions;
...
export type ImageViewProps = ImageViewSourceProps | { state: ImagePaintState };

export function createImagePaintState({
	base64Data,
	mimeType,
	theme,
	options = {},
	dimensions,
}: ImageViewSourceProps): ImagePaintState {
	const budget = options.budget;
	return {
		base64Data,
		mimeType,
		dimensions: dimensions || getImageDimensions(base64Data, mimeType) || { widthPx: 800, heightPx: 600 },
		theme,
		options,
		budget,
		imageId: budget ? budget.acquireId(options.imageKey) : undefined,
		cache: new RichText(),
		hasCache: false,
...
export function ImageView(props: ImageViewProps) {
	return <image state={"state" in props ? props.state : createImagePaintState(props)} />;
}
