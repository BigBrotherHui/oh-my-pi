...
import { Attr, DEFAULT_COLOR, rgb, Style } from "../core/style";
...
import { hasMagicKeyword, magicKeywordRuns } from "./magic-keywords";
 ...
 * native `borderStyle` and a `symbols` object — so a `TUI` passed in the first
...
function isEditorTheme(value: unknown): value is EditorTheme {
	...
	return candidate.borderStyle instanceof Style && typeof candidate.symbols === "object" && candidate.symbols !== null;
}
...
export interface CustomEditorViewProps {
	key?: NodeKey;
	editor: CustomEditor;
	/** Retained status content embedded in the editor's native top chrome. */
	topBorder?: JSX.Element;
	cursor?: EditorCursor;
	/** Read-only sample fitted by native editor geometry. */
	previewText?: string;
	/** Negative values keep static preview editors out of the focus order. */
	tabIndex?: number;
}

/** Run-based custom-editor view for TSX consumers. */
export function CustomEditorView(props: CustomEditorViewProps) {
	return (
		<EditorView
			editor={props.editor}
...
export class CustomEditor extends Editor {
	#spelling: MacOSSpellingProvider;
	 ...
	 * (it carries `symbols`/`borderStyle`) rather than by position, and capture a
	...
	constructor(...args: readonly unknown[]) {
		...
		this.#spelling = args.find(arg => arg instanceof MacOSSpellingProvider) ?? new MacOSSpellingProvider();
		const requestTextAssistRepaint = (): void => this.invalidate();
	...
	/** Run-native decoration path used by the ported editor. */
	override decorateRuns = (sink: Out, text: string, context: EditorTextDecorationContext, base: Style): void => {
		this.#syncComposerTokenPattern();
		...
		const decorateProse = (target: Out, value: string, sourceOffset: number): void => {
			const spellingRuns = this.#spelling.typoRuns(
				value,
				{
					editorText,
			...
			for (const spelling of spellingRuns) {
				for (const run of magicKeywordRuns(spelling.text, spelling.style.withFg(DEFAULT_COLOR), phase)) {
					target.push(run.style, run.text);
				}
			}
		};
		visitPlaceholders(
			text,
			{
				text: value => {
					...
					if (this.#queueShorthandActive && (value.startsWith("->") || value.startsWith("=>"))) {
						...
						sink.push(
							typeof theme === "undefined" ? base.plus(Attr.Dim) : theme.style("dim"),
							`Queueing ${icon}`,
						);
						decorateProse(sink, value.slice(2), sourceOffset + 2);
						return;
					}
					if (this.#queueListActive) {
						...
						if (markerMatch) {
							...
							if (indent) sink.push(base, indent);
							const markerStyle = typeof theme === "undefined" ? base : theme.style("accent");
							sink.push(markerStyle, value.slice(indent.length, markerEnd));
							decorateProse(sink, value.slice(markerEnd), sourceOffset + markerEnd);
							return;
						}
					}
					decorateProse(sink, value, sourceOffset);
				},
				skill: (label, name) => {
					locateSource(label);
					let tokenStyle =
						typeof theme === "undefined"
							? base.plus(Attr.Bold)
							: theme.style("customMessageLabel", "customMessageBg").plus(Attr.Bold);
					const filePath = this.skillFilePath(name);
					if (filePath !== undefined) tokenStyle = fileHyperlinkStyle(filePath, { line: 1 }, tokenStyle);
					sink.push(tokenStyle, label);
				},
				mention: label => {
					locateSource(label);
					const mentionStyle =
						typeof theme === "undefined"
							? base.plus(Attr.Bold)
							: theme.style("statusLineModel", "customMessageBg").plus(Attr.Bold);
					sink.push(mentionStyle, label);
				},
				reference: (value, kind, index, form) => {
					locateSource(value);
					const [red, green, blue] = attachmentRgb(kind, index);
					let referenceStyle =
						form === "chip"
							? Style.of({ fg: rgb(red, green, blue) }).plus(Attr.Bold)
							: (typeof theme === "undefined" ? base : theme.style("accent")).plus(Attr.Bold);
					if (form === "marker" && (kind === "image" || kind === "video")) {
						referenceStyle = referenceStyle.plus(Attr.Underline);
					}
					if (kind === "image" || kind === "video") {
						const target = this.imageLinks?.[index - 1];
						if (target) referenceStyle = fileHyperlinkStyle(target, undefined, referenceStyle);
					}
					sink.push(referenceStyle, value);
				},
			},
	...
	imageReferenceHyperlink: (
		...
		renderLabel: (labelText: string) => string,
