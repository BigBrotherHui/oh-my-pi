...
import { Attr, DEFAULT_COLOR, parseColor, Style, type Color } from "../core/style";
import { createEffect, createSignal, Show, type Accessor, type JSX, type Setter, useClock } from "../reactive";
...
import { getToolSchemaMetadataRevision } from "./context-usage";
import {
...
import { statusSegmentNode, type SegmentContext } from "./segments";
import { getSeparator, powerlineTransitionStyle } from "./separators";
import type {
	CollabStatus,
...
function resolveWorktreeContext(cwd: string): WorktreeContext | null {
	...
	const base = basename(worktree.primaryRoot);
	const projectName = base.endsWith(".git") ? base.slice(0, -4) : base;
	if (!projectName) return null;
	return { projectName, worktreeName: basename(worktree.root) };
}
...
export type StatusLineLayout = "box" | "band" | "plain-full" | "plain-left" | "plain-right";
...
export interface StatusLineSource {
	...
	): "frame" | "spinner" | "second" | undefined;
	previewLayout?(): StatusLineLayout;
	view(
...
export interface StatusLineProps {
	...
	/** Render a one-sided preview group at intrinsic width instead of spanning its host row. */
	readonly naturalWidth?: boolean;
}

function StatusLineAtClock(props: StatusLineProps & { readonly cadence: "frame" | "spinner" | "second" }): JSX.Element {
	const now = useClock(props.cadence);
	const [tick, setTick] = createSignal(now());
	createEffect(() => setTick(now()));
	return (
		<sized
			paint={width => (
				<Show when={tick()} keyed>
					{(timestamp: number) =>
						props.source.view(
							timestamp,
							props.layout,
...
/**
 * Declarative status-line view. Its owning effect bridges the domain revision
 * signal into a keyed retained scope, which mounts or disposes the root clock.
 */
export function StatusLine(props: StatusLineProps): JSX.Element {
	const [presentation, setPresentation] = createSignal({
		revision: -1,
		cadence: undefined as "frame" | "spinner" | "second" | undefined,
	});
	createEffect(() => {
		const revision = props.source.revision();
		setPresentation({ revision, cadence: props.source.clockCadence(props.layout, props.placeholders) });
	});
	return (
		<Show when={presentation()} keyed>
			{(state: { revision: number; cadence: "frame" | "spinner" | "second" | undefined }) =>
				state.cadence ? (
					<StatusLineAtClock {...props} cadence={state.cadence} />
				) : (
					<sized
						paint={width =>
							props.source.view(
...
/** Sanitize dynamic status content so one entry cannot create extra terminal rows. */
export function flattenStatusLineText(text: string): string {
	return sanitizeStatusText(text).replace(/[\r\n]+/g, " ");
}
...
/** Declarative status-line view for the active composer placement. */
export function StatusLineView(props: StatusLineViewProps): JSX.Element {
	return <StatusLine source={props.source} />;
}

/** Declarative composer-preview view. The caller selects the preview layout directly. */
export function StatusLinePreviewView(
	props: StatusLineViewProps & {
		readonly layout?: StatusLineLayout;
		readonly previewTitle?: string;
		/** Keep a top-rule chip at its natural width for editor chrome placement. */
		readonly naturalWidth?: boolean;
	},
): JSX.Element {
	return (
		<StatusLine
			source={props.source}
			layout={props.layout ?? props.source.previewLayout?.() ?? "box"}
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements StatusLineSource {
	...
	// Bumped on every branch-cache reset (#refreshGitSnapshot — a HEAD move or
	...
	// bookmark move (#refreshGitSnapshot). An in-flight jj query captures this
	...
	/** Publish the current session snapshot after the host has ingested an event. */
	ingestSession(): void {
		if (this.#disposed) return;
		// A tool can open, close, or merge a PR without moving HEAD. The next
		// domain read must therefore resolve the settled PR context again.
		this.#cachedPrContext = undefined;
		this.#refreshPresentationData();
		this.#publish();
	}

	/** Whether the attached session is currently accumulating active processing time. */
	isActivityActive(): boolean {
		return !this.#disposed && this.#meter().activeStartedAt !== null;
	}

	/** Selects the root clock only when a configured visible segment needs it. */
	clockCadence(
		layout: StatusLineLayout | undefined,
		placeholders: boolean | undefined,
	): "frame" | "spinner" | "second" | undefined {
		...
		const leftSegments = layout === "plain-right" ? [] : effectiveSettings.leftSegments;
		const rightSegments = layout === "plain-left" ? [] : effectiveSettings.rightSegments;
		if (this.isActivityActive() || this.session.compactionSpeculation === "running") {
			return "spinner";
		}
		if (this.#brandFade !== null) return "frame";
		if (
			...
			leftSegments.includes("time") ||
			rightSegments.includes("time")
		) {
			return "second";
		}
		return undefined;
	...
	#resolveActiveRepoCache(): ActiveRepoCache {
		...
		this.#activeRepoCache = {
			...
			repository,
			displayRepository,
			displayRepositoryCheckedAt: currentTimeMs(),
			repositoryCheckedAt: currentTimeMs(),
			worktree,
		};
		return this.#activeRepoCache;
	}
	...
	setSession(session: TSession, focusedAgentId?: string): void {
		...
		if (sessionChanged) {
			this.#resetSessionSnapshotCaches();
	...
	markActivityStart(): void {
		const meter = this.#meter();
		if (meter.activeStartedAt !== null) return;
		meter.activeStartedAt = currentTimeMs();
		this.#startBrandFade(true);
		this.#publish();
	}
	...
	markActivityEnd(): void {
		const meter = this.#meter();
		if (meter.activeStartedAt === null) return;
		meter.activeMs += Math.max(0, currentTimeMs() - meter.activeStartedAt);
		meter.activeStartedAt = null;
		this.#startBrandFade(false);
		this.#publish();
	}
	 ...
	 */
	getActiveMs(nowMs = currentTimeMs()): number {
		const meter = this.#meter();
		if (meter.activeStartedAt === null) return meter.activeMs;
		return meter.activeMs + Math.max(0, nowMs - meter.activeStartedAt);
	}
	/**
	 ...
	 */
	getTurnElapsedMs(nowMs = currentTimeMs()): number | null {
	...
	/** Record a domain working-state edge; the mounted root clock drives interpolation. */
	#startBrandFade(working: boolean): void {
		this.#brandFgColor(working, this.#resolveSettings().sessionAccent !== false, currentTimeMs());
	}

	/**
	 * Foreground for the `pi` brand segment. Active interpolation is sampled
	 * against the root clock supplied to the view, never a view-local clock.
	 */
	#brandFgColor(working: boolean, sessionAccentEnabled: boolean, now: number): Color {
		...
		try {
			...
			return theme.fgColor(working ? "accent" : "dim");
		}
	}
	...
	#syncPricingTimer(): void {
		...
		if (this.#disposed || !cost?.timeBased || !costVisible) {
			this.#stopPricingTimer();
			return;
		}
		const now = currentTimeMs();
	...
	#sessionAccentColor(enabled: boolean): Color | undefined {
		if (!enabled) return undefined;
		const name = this.session.sessionManager?.getSessionName();
		if (!name) return undefined;
		try {
			return parseColor(getSessionAccentHex(name, theme.sessionAccentInputs));
		} catch {
			return undefined;
		}
	}
	...
	#publish(observedAt = currentTimeMs()): void {
		this.#observedAt = observedAt;
		this.#publishRevision(revision => revision + 1);
	}
	#resetSessionSnapshotCaches(): void {
		this.#clearUsageStartTimer();
	...
	/**
	 * Ingest a VCS-head or repository transition. Aborts superseded resolves,
	 * retires stale branch/PR/jj domain values, then schedules fresh domain reads.
	 */
	...
	/**
	 * Re-point the VCS watcher and domain cache at a new cwd/repository.
	 * The watcher follows the effective repository, so an old workspace cannot
	 * publish a stale branch into the new session.
	 */
	applyCwdChange(): void {
		this.#retireGitWatcher();
		this.refreshGitSnapshot();
	...
	#getBranchLabel(
		...
		if (!gitRepository) {
			if (this.#jjBranchActive || currentTimeMs() - this.#jjBranchLastFetch < JJ_REFRESH_TTL_MS) {
			...
			(async () => {
				...
				try {
					...
					if (this.#jjCacheGeneration === generation) this.#jjBranchLastFetch = currentTimeMs();
				}
		...
		const fallbackCacheExpired =
			...
			(this.#branchLastFetch === undefined ||
				currentTimeMs() - this.#branchLastFetch >= WATCHER_FAILURE_POLL_TTL_MS);
		...
		if (repoInfo?.isReftable) {
			...
			// Capture the cache generation at launch. refreshGitSnapshot bumps it
			...
			(async () => {
				...
				this.#branchLastFetch = currentTimeMs();
				if (prev !== next) {
					this.#publish();
				}
			})();
	...
	#getStatus(activeRepoCache: ActiveRepoCache = this.#resolveActiveRepoCache()): {
		...
		if (repository.kind() === "jj") {
			if (this.#jjStatusActive || currentTimeMs() - this.#jjStatusLastFetch < JJ_REFRESH_TTL_MS) {
			...
			(async () => {
				...
				try {
					...
					if (this.#jjCacheGeneration === generation) this.#jjStatusLastFetch = currentTimeMs();
				}
				if (this.#jjCacheGeneration !== generation || this.#disposed) return;
		...
		if (this.#cachedGitStatusCwd === gitCwd && currentTimeMs() - this.#gitStatusLastFetch < 1000) {
		...
		(async () => {
			...
			try {
				...
				if (this.#gitStatusInFlightCwd === gitCwd) {
					...
					this.#gitStatusLastFetch = currentTimeMs();
	...
	async #runUsageRefresh(session: TSession, usageContextKey: string): Promise<void> {
		if (this.#disposed || this.session !== session || this.#getUsageContextKey(session) !== usageContextKey) {
			if (this.#usageInFlightContextKey === usageContextKey) {
				this.#usageInFlight = false;
				this.#usageInFlightContextKey = null;
			}
		...
		try {
			...
			if (this.session !== session || this.#getUsageContextKey(session) !== usageContextKey) return;
			this.#usageFetchedAt = currentTimeMs();
			if (signal.aborted && reportsPromise) {
				this.#observeLateUsageRefresh(session, reportsPromise, sequence, usageContextKey);
			}
		} finally {
			if (this.session === session && this.#usageInFlightContextKey === usageContextKey) {
				this.#usageInFlight = false;
				this.#usageInFlightContextKey = null;
			}
		}
	...
	#applyUsageRefreshReports(session: TSession, reports: unknown, sequence: number, usageContextKey: string): void {
		...
		this.#usageFetchedAt = currentTimeMs();
		// Async usage ingestion publishes the completed snapshot immediately.
		if (usageChanged) {
			this.#publish();
		}
	...
	#buildSegmentContext(
		...
		const activeRepoCache = this.#activeRepoCache;
		...
		const turnElapsedMs = this.getTurnElapsedMs(nowMs);
		return {
			session: this.session,
			focusedAgentId: this.#focusedAgentId,
			sessionAccent: sessionAccentEnabled,
			previewTitle,
			activeRepo: activeRepoCache?.activeRepo ?? null,
			width: 0,
			...
			speculationBlinkOn: Math.floor(nowMs / 600) % 2 === 0,
			subagentCount: this.#subagentCount,
			activeMs: this.getActiveMs(nowMs),
			turnElapsedMs,
			now: new Date(nowMs),
			brandFg: this.#brandFgColor(turnElapsedMs !== null, sessionAccentEnabled, nowMs),
			git: {
				branch: this.#cachedJjBranch ?? this.#cachedBranch ?? null,
				status: this.#cachedJjStatus ?? this.#cachedGitStatus,
				pr: this.#cachedPr ?? null,
			},
			worktree: activeRepoCache?.worktree ?? null,
	...
	#refreshPresentationData(): void {
		const settings = this.#resolveSettings();
		const includePath = hasPathSegment(settings.leftSegments) || hasPathSegment(settings.rightSegments);
		const includeGit = hasGitSegment(settings.leftSegments) || hasGitSegment(settings.rightSegments);
		const includePr = hasPrSegment(settings.leftSegments) || hasPrSegment(settings.rightSegments);
		this.refreshUsageInBackground();
		if (this.#gitEnabled() && (includePath || includeGit || includePr)) {
			const activeRepo = this.#resolveActiveRepoCache();
			if (includeGit || includePr) this.#getBranchLabel(activeRepo);
			if (includeGit) this.#getStatus(activeRepo);
			if (includePr) this.#lookupPr(activeRepo);
		}
		this.#syncPricingTimer();
	}
	...
	#statusLineNode(
		...
		const left = layout === "plain-right" ? [] : [...settings.leftSegments];
		const right = layout === "plain-left" ? [] : [...settings.rightSegments];
		if (layout === "band") {
			const sessionName = right.indexOf("session_name");
			if (sessionName >= 0) right.splice(sessionName, 1);
		}
		if (
			previewTitle !== undefined &&
			width !== undefined &&
			width <= 40 &&
			(layout === "box" || layout === "band" || layout === "plain-full")
		) {
			right.length = 0;
			if (layout === "box") {
				const mode = left.indexOf("mode");
				if (mode >= 0) left.splice(mode, 1);
			}
		}
		const containsContext = (segments: readonly StatusLineSegmentId[]): boolean =>
			segments.includes("context_pct") || segments.includes("context_total");
		const containsNonContext = (segments: readonly StatusLineSegmentId[]): boolean =>
			segments.some(segment => segment !== "context_pct" && segment !== "context_total");
		const context = this.#buildSegmentContext(settings.segmentOptions, nowMs, previewTitle);
		const embedContext =
			!plain &&
			settings.contextLine === "embedded" &&
			context.contextPercent !== null &&
			context.contextWindow > 0 &&
			(containsContext(left) || containsContext(right)) &&
			(containsNonContext(left) || containsNonContext(right));
		if (embedContext) {
			for (const segments of [left, right]) {
				for (let index = segments.length - 1; index >= 0; index--) {
					if (segments[index] === "context_pct" || segments[index] === "context_total") segments.splice(index, 1);
				}
			}
		}
		...
		if (layout !== "plain-left") {
			...
			if (backgroundJobs > 0) {
				badges.push(
					<span style={theme.style("statusLineSubagents")}>
						{theme.icon.job} {placeholders ? "…" : backgroundJobs}
					</span>,
				);
			}
			if (this.#subagentCount > 0) {
				badges.push(
					<span style={theme.style("statusLineSubagents")}>
						{theme.icon.agents} {placeholders ? "…" : this.#subagentCount}
					</span>,
				);
			}
		}
		const leftGroup = this.#groupNode(left, "left", context, settings, layout, plain, placeholders, [], naturalWidth);
		const rightGroup = this.#groupNode(
			right,
			"right",
			context,
			settings,
		...
		if (plain) {
			if (naturalWidth) return leftGroup ?? rightGroup;
			return (
				<row>
					{leftGroup ? <box shrink={1}>{leftGroup}</box> : null}
					<text grow={1} wrap="none" />
					{rightGroup ? <box shrink={1}>{rightGroup}</box> : null}
				</row>
			);
		}
		return (
			<row>
				{leftGroup ? <box shrink={1}>{leftGroup}</box> : null}
				{this.#contextGaugeNode(context, settings, embedContext, placeholders)}
				{rightGroup ? <box shrink={1}>{rightGroup}</box> : null}
			</row>
		);
	}
	...
	#groupNode(
		...
		plain: boolean,
		placeholders: boolean,
		prefix: readonly JSX.Element[] = [],
		previewChip = false,
	): JSX.Element | null {
		const content = [
			...prefix.map((node, index) => ({ id: `badge:${index}`, node })),
			...segments
				.map(id => ({
					id,
					node: statusSegmentNode(id, placeholders ? { ...context, startupPlaceholder: true } : context),
				}))
				.filter((part): part is { id: StatusLineSegmentId; node: JSX.Element } => part.node !== null),
		];
		if (content.length === 0) return null;
		const separator = plain
			? "·"
			: direction === "left"
				? getSeparator(settings.separator ?? "powerline-thin", theme).left
				: getSeparator(settings.separator ?? "powerline-thin", theme).right;
		const background =
			(plain && !previewChip) || settings.transparent ? DEFAULT_COLOR : theme.bgColor("statusLineBg");
		const attrs = this.#focusedAgentId ? Attr.Dim : Attr.None;
		const base = Style.of({ fg: theme.fgColor("text"), bg: background, attrs });
		const neutral = Style.of({ bg: background, attrs });
		const sessionAccent = this.#sessionAccentColor(settings.sessionAccent !== false);
		const divider = Style.of({ fg: sessionAccent ?? theme.fgColor("statusLineSep"), bg: background, attrs });
		const separatorDef = plain ? undefined : getSeparator(settings.separator ?? "powerline-thin", theme);
		const cap =
			separatorDef?.endCaps && background !== DEFAULT_COLOR
				? direction === "left"
					? separatorDef.endCaps.right
					: separatorDef.endCaps.left
				: "";
		const capStyle = powerlineTransitionStyle(background, DEFAULT_COLOR);
		const bandCap =
			direction === "left" && layout === "band" && separatorDef?.endCaps && background !== DEFAULT_COLOR;
		return (
			<row shrink={1}>
				{direction === "right" && cap ? (
				...
				{bandCap ? (
					<text shrink={0} wrap="none">
						<span style={capStyle}>{theme.sep.powerlineCapLeft}</span>
					</text>
				) : null}
				<text shrink={0} wrap="none">
				...
				{content.map((part, index) => (
					<text
						...
						wrap="none"
						overflow="ellipsis"
						overflowPriority={part.id === "model" ? 0 : part.id === "path" ? 2 : 1}
					>
						{index > 0 ? (
							<>
								<span style={neutral}> </span>
								<span style={divider}>{separator}</span>
								<span style={base}> </span>
							</>
						) : null}
						<span style={base}>{part.node}</span>
					</text>
				))}
				<text shrink={0} wrap="none">
					<span style={neutral}> </span>
	...
	#contextGaugeNode(
		...
		return (
			<progress
				grow={1}
				render={width => this.#contextGaugeRuns(width, context, settings, embedContext, placeholders)}
			/>
		);
	}

	/** Build the exact allocated-width context gauge, including compaction markers and embedded labels. */
	#contextGaugeRuns(
		width: number,
		context: SegmentContext,
		settings: EffectiveStatusLineSettings,
		embedContext: boolean,
		placeholders: boolean,
	): readonly { text: string; color: Color }[] {
		const horizontal = theme.boxRound.horizontal;
		const percent = context.contextPercent;
		const sessionName = settings.sessionAccent !== false ? this.session.sessionManager?.getSessionName() : undefined;
		const accentHex = sessionName ? getSessionAccentHex(sessionName, theme.sessionAccentInputs) : undefined;
		const usedColor = parseColor(accentHex ?? theme.getColorHex("borderAccent"));
		if (settings.contextLine === "off" || percent === null)
			return [{ text: horizontal.repeat(width), color: usedColor }];
		...
		if (embedContext) {
			const candidatePercent = placeholders
				? "…%"
				: formatEmbeddedContextPercent(overflow ? percent : clampedPercent);
			const candidateWindow = placeholders ? "…" : formatNumber(context.contextWindow);
			if (width >= candidatePercent.length + candidateWindow.length + 4) {
				percentLabel = candidatePercent;
				windowLabel = candidateWindow;
				if (overflow) {
					percentStart = width - percentLabel.length;
					windowStart = percentStart - 1 - windowLabel.length;
				} else {
					windowStart = width - windowLabel.length - 1;
				}
				scaleWidth = windowStart;
			}
		}

		const usedCount = Math.min(scaleWidth, Math.max(1, Math.round((clampedPercent / 100) * scaleWidth)));
		let speculationIndex = -1;
		let thresholdIndex = -1;
		const mode = settings.contextLine ?? "embedded";
		if ((mode === "annotated" || mode === "embedded") && context.autoCompactEnabled && width >= 8) {
			const boundaries = this.#compactionBoundaries(context.contextWindow);
			if (boundaries) {
				const cellFor = (value: number) =>
					Math.min(scaleWidth - 1, Math.max(0, Math.round((value / 100) * scaleWidth)));
				thresholdIndex = cellFor(boundaries.thresholdPercent);
				if (boundaries.speculationPercent !== null) speculationIndex = cellFor(boundaries.speculationPercent);
		...
		if (percentLabel && percentStart < 0) {
			...
			const overlapsBoundary = (start: number): boolean => {
				...
				return (
					(speculationIndex >= start && speculationIndex < end) ||
					(thresholdIndex >= start && thresholdIndex < end)
				);
			};
		...
		const unusedColor = parseColor(theme.getColorHex("border"));
		const thresholdColor = parseColor(adjustHsv(accentHex ?? theme.getColorHex("borderAccent"), { s: 0.7, v: 0.75 }));
		const speculationColor = theme.fgColor("muted");
		const overflowColor = theme.fgColor("error");
		...
		for (let index = 0; index < width; index++) {
			...
			let color = index < usedCount ? usedColor : unusedColor;
			if (percentStart >= 0 && index >= percentStart && index < percentStart + percentLabel.length) {
				glyph = percentLabel.charAt(index - percentStart);
				color = overflow ? overflowColor : usedColor;
	...
	/** Auto-compaction boundary percents, or null when unavailable. */
	#compactionBoundaries(contextWindow: number) {
	 ...
	 * the editor's top rule; `"none"` embeds both in the editor's top border.
	 */
	setComposerStyle(style: Pick<ComposerStyle, "statusAttachment" | "bottomBar" | "bottomBarGap">): void {
		const standalone = style.bottomBar === "none" ? false : style.bottomBar === "left" ? "left-only" : "full";
		const changed =
	...
	setStandalone(mode: false | "full" | "left-only", gap = false): void {
		if (this.#standalone === mode && this.#standaloneGap === gap) return;
		this.#standalone = mode;
		this.#standaloneGap = gap;
		this.#syncPricingTimer();
		this.#publish();
	}
	...
	/** Current top attachment for settings/setup previews. */
	previewLayout(): StatusLineLayout {
		if (this.#topAttachment === "top-band") return "band";
		if (this.#topAttachment === "top-rule-chip") return "plain-right";
		return "box";
	}

	/** Declarative retained view for a composer status placement. */
	view(
		nowMs: number | undefined,
		layout?: StatusLineLayout,
		previewTitle?: string,
		...
		if (!this.#disposed) this.#refreshPresentationData();
		if (nowMs !== undefined && this.#brandFade && nowMs - this.#brandFade.startedAt >= BRAND_FADE_MS) {
			this.#brandFade = null;
			this.#publish(nowMs);
		}
		const resolvedLayout =
			layout ??
			(this.#standalone === "left-only" ? "plain-left" : this.#standalone === "full" ? "plain-full" : undefined);
		const status =
			resolvedLayout && (layout !== undefined || !this.#autocompleteActiveProbe?.())
				? this.#statusLineNode(
						resolvedLayout,
						previewTitle,
						placeholders,
						nowMs ?? this.#observedAt,
						naturalWidth,
						width,
					)
				: null;
		const showHooks = layout === undefined && (this.#settings.showHookStatus ?? true);
		return (
			<stack>
				{status && layout === undefined && this.#standaloneGap ? <br /> : null}
				{status}
				{showHooks
					? this.#sortedHookStatuses.map((text, index) => (
							<text key={index} wrap="clip">
								{flattenStatusLineText(text)}
							</text>
						))
					: null}
			</stack>
