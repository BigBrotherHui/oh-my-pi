...
function ContextGridRow(props: { cells: readonly CellSpec[]; row: number }): JSX.Element {
	return (
		<>
			{Array.from({ length: GRID_COLS }, (_, col) => {
				const cell = props.cells[props.row * GRID_COLS + col]!;
				return (
					<span key={col}>
						{col > 0 ? " " : null}
						<span color={cell.color}>{cell.glyph}</span>
					</span>
				);
			})}
		</>
...
function contextLegendRows(props: { breakdown: ContextBreakdown }): JSX.Element[] {
	const breakdown = props.breakdown;
	const model = breakdown.model;
	const contextWindow = breakdown.contextWindow;
	const categories = breakdown.categories;
	const usedTokens = breakdown.usedTokens;
	const autoCompactBufferTokens = breakdown.autoCompactBufferTokens;
	const freeTokens = breakdown.freeTokens;
	const modelName = model?.name ?? model?.id ?? "no model";
	const modelId = model?.id ?? "unknown";
	const windowLabel = formatNumber(contextWindow).toLowerCase();
	const rows: JSX.Element[] = [
		<span key="model">
			<span bold>{modelName}</span>
			<span color="dim"> ({windowLabel} context)</span>
		</span>,
		<span key="id" color="muted">
			{modelId}[{windowLabel}]
		</span>,
		<span key="used">
			<span bold>{formatNumber(usedTokens)}</span>
			<span color="dim">/{windowLabel} tokens</span>
			<span color="muted"> ({percentString(usedTokens, contextWindow)})</span>
		</span>,
		null,
		<span key="category-title" color="muted">
	...
	for (const category of categories) {
		rows.push(
			<span key={`category:${category.id}`}>
				<span color={category.color}>{category.glyph}</span> {category.label}:{" "}
				<span bold>{formatNumber(category.tokens)}</span>{" "}
				<span color="dim">tokens ({percentString(category.tokens, contextWindow)})</span>
			</span>,
		);
	}
	rows.push(
		<span key="free">
			<span color="dim">{CELL_FREE}</span> Free space: <span bold>{formatNumber(freeTokens)}</span>{" "}
			<span color="dim">({percentString(freeTokens, contextWindow)})</span>
		</span>,
	);
	if (autoCompactBufferTokens > 0) {
		rows.push(
			<span key="buffer">
				<span color="warning">{CELL_BUFFER}</span> Autocompact buffer:{" "}
				<span bold>{formatNumber(autoCompactBufferTokens)}</span>{" "}
				<span color="dim">tokens ({percentString(autoCompactBufferTokens, contextWindow)})</span>
			</span>,
		);
	}
	const snap = props.breakdown.snapcompact;
	if (snap) {
		rows.push(null);
		if (!snap.visionCapable) {
			rows.push(
				<span key="snap-inactive" color="muted">
					Snapcompact: inactive (model has no image input)
				</span>,
			);
		} else {
			rows.push(
				<span key="snap-title" color="muted">
			...
			if (sp) {
				...
				rows.push(
					<span key="snap-system">
						{"  "}System prompt ({sp.scope === "agents-md" ? "AGENTS.md" : "all"}):{" "}
						{sp.applied ? (
							<>
								saves <span bold>~{formatNumber(sp.savedTokens)}</span>{" "}
								<span color="dim">
									({formatNumber(sp.textTokens)} text → {sp.frames} frame{sp.frames === 1 ? "" : "s"} ≈{" "}
									{formatNumber(sp.imageTokens)})
								</span>
							</>
						) : (
							<span color="dim">stays text ({reason})</span>
						)}
					</span>,
				);
			}
			...
			if (tr) {
				rows.push(
					<span key="snap-tools">
						...
						{tr.swapped > 0 ? (
							<>
								saves <span bold>~{formatNumber(tr.savedTokens)}</span>{" "}
								<span color="dim">
									({tr.swapped}/{tr.total} imaged, {formatNumber(tr.textTokens)} text → {tr.frames} frames ≈{" "}
									{formatNumber(tr.imageTokens)})
								</span>
							</>
						) : (
							<span color="dim">none imaged ({tr.total} in history)</span>
						)}
					</span>,
				);
			}
			if (snap.savedTokens > 0) {
				rows.push(
					<span key="snap-next">
						{"  "}Next request: <span bold>~{formatNumber(Math.max(0, usedTokens - snap.savedTokens))}</span>{" "}
						<span color="dim">tokens on the wire</span>
					</span>,
				);
			}
		}
...
/** Colorful context-usage grid and legend. */
export function ContextUsageView(props: ContextUsageViewProps): JSX.Element {
	if (props.breakdown.contextWindow <= 0) {
		return <text color="muted">Context usage is unavailable: no model is selected for this session.</text>;
	}
	const cells = planCells(props.breakdown);
	const legend = contextLegendRows({ breakdown: props.breakdown });
	const rows = Math.max(GRID_ROWS, legend.length);
	return (
		<stack>
			{Array.from({ length: rows }, (_, row) => (
				<text key={row} wrap="none">
					{row < GRID_ROWS ? <ContextGridRow cells={cells} row={row} /> : " ".repeat(GRID_COLS * 2 - 1)}
					{legend[row] === null || legend[row] === undefined ? null : (
						<>
							{GRID_GUTTER}
							{legend[row]}
						</>
...
/** Build the context-usage view without crossing the terminal string boundary. */
export function renderContextUsage(breakdown: ContextBreakdown, uiTheme: Theme): JSX.Element {
	return <ContextUsageView breakdown={breakdown} uiTheme={uiTheme} />;
}
