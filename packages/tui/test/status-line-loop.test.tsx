...
import { expectSameCells } from "./cell-grid";
...
describe("status line loop mode segment", () => {
	it("shows that a bounded loop is waiting for its first prompt", () => {
		...
		expect(Bun.stripANSI(renderVNode(rendered.content))).toBe(withIcon(theme.icon.loop, "Loop waiting 10/10"));
	});
	...
	it("shows the live remaining duration while a loop is running", () => {
		...
		expect(Bun.stripANSI(renderVNode(rendered.content))).toBe(withIcon(theme.icon.loop, "Loop running 1m30s left"));
	});
	...
	it("distinguishes a paused loop from an active loop", () => {
		...
		expect(Bun.stripANSI(renderVNode(rendered.content))).toBe(withIcon(icon, "Loop paused"));
		expectSameCells([theme.fg("warning", withIcon(icon, "Loop paused"))], [renderVNode(rendered.content)], 120);
	});
	...
	it("shows the gating condition alongside the remaining iterations", () => {
		...
		expect(Bun.stripANSI(renderVNode(rendered.content))).toBe(
			withIcon(theme.icon.loop, "Loop running 3/3 until: bun test"),
		);
	});
	...
	it("bounds a long condition command", () => {
		...
		const rows = renderToRows(
			() => (
				<row gap={1}>
					<text minWidth={0} grow={1} shrink={1} wrap="none" overflow="ellipsis">
						{rendered.content}
					</text>
					<text shrink={0}>Next segment</text>
				</row>
			),
			50,
		).map(row => Bun.stripANSI(row));
		expect(rows).toHaveLength(1);
		expect(rows[0]).toContain("Loop running while: bun test");
		expect(rows[0]).toContain("Next segment");
		expect(Bun.stringWidth(rows[0]!)).toBeLessThanOrEqual(50);
	});
});
